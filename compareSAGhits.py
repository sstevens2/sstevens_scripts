...
input['READ']=input[0].str.split('.blast:').str.get(1)
# Add a new column that is just the SAG name from the 2nd[2] column
input['SAG']=input[1].str.split('_').str.get(0) # need if using value counts later
...
# Assert above might fail if you are looking at low identity reads and didn't filter for best hits first,
# either filter for bbh or remove assert for that run.
        ref2_df=input[input['SAG'] == ref2]
